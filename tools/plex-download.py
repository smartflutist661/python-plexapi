...
def search_for_item(url=None):
    if url:
        return get_item_from_url(opts.url)
    ...
    items = utils.choose(
        "Choose result", items, lambda x: "(%s) %s" % (x.type.title(), x.title[0:60])
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-u", "--username", help="Your Plex username", default=CONFIG.get("auth.myplex_username")
    )
    parser.add_argument(
        "-p", "--password", help="Your Plex password", default=CONFIG.get("auth.myplex_password")
    )
