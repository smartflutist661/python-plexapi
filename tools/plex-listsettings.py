...
def _setting_group(setting):
    if not setting.label and not setting.summary:
        return "undocumented"
    if setting.group == "":
        return "misc"
    ...
    for setting in groups[group]:
        ...
        if group == "undocumented":
            ...
            if any((setting.default, setting.enumValues)):
                handle.write(": ")
                if setting.default:
                    handle.write("default: %s" % setting.default)
                if all((setting.default, setting.enumValues)):
                    handle.write("; ")
                if setting.enumValues:
                    handle.write("choices: %s" % setting._data.attrib["enumValues"])
            handle.write("\n")
            continue
        ...
        if any((setting.label, setting.summary, setting.default, setting.enumValues)):
            handle.write("  ")
            if setting.label:
                handle.write("%s." % setting.label)
            if setting.summary:
                handle.write(" %s" % setting.summary)
            if any((setting.default, setting.enumValues)):
                handle.write(" (")
                if setting.default:
                    handle.write("default: %s" % setting.default)
