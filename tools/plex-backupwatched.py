...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "action", help="Action to perform: backup or restore", choices=("backup", "restore")
    )
    parser.add_argument("filepath", help="File path to backup to or restore from")
    parser.add_argument(
        "-u", "--username", default=CONFIG.get("auth.myplex_username"), help="Plex username"
    )
    parser.add_argument(
        "-p", "--password", default=CONFIG.get("auth.myplex_password"), help="Plex password"
    )
    ...
    parser.add_argument(
        "-l",
        "--libraries",
        help="Only backup or restore the specified libraries (comma separated).",
    )
