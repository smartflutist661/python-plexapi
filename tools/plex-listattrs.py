...
class PlexAttributes:
    ...
    def __init__(self, opts):
        ...
        self.clsnames = [
            c for c in opts.clsnames.split(",") if c
        ]  # list of clsnames to report (blank=all)
    ...
    def _parse_objects(self, obj, cat=None):
        ...
        self._load_xml_attrs(
            clsname,
            obj._data,
            self.attrs[clsname]["xml"],
            self.attrs[clsname]["examples"],
            self.attrs[clsname]["categories"],
            cat,
        )
        self._load_obj_attrs(clsname, obj, self.attrs[clsname]["obj"], self.attrs[clsname]["docs"])
    ...
    def _load_obj_attrs(self, clsname, obj, attrs, docs):
        if clsname in STOP_RECURSING_AT:
            return None
        ...
        for clsname in sorted(self.attrs.keys()):
            if self._clsname_match(clsname):
                ...
                for attr in attrs:
                    ...
                    print(
                        "%7s  %3s  %-30s  %-20s  %s" % (count, state, attr, categories, examples)
                    )
                    total_attrs += count
        print(_("\nSUMMARY\n%s" % ("-" * 30), "yellow"))
        print(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="force a full refresh of attributes.",
    )
    parser.add_argument("-m", "--max", default=99999, help="max number of objects to load.")
    parser.add_argument(
        "-e", "--examples", default=10, help="max number of example values to load."
    )
    parser.add_argument(
        "-c", "--clsnames", default="", help="only report the following class names"
    )
