...
class Video(PlexPartialObject, PlayedUnplayedMixin):
    ...
    def uploadSubtitles(self, filepath):
        ...
        with open(filepath, "rb") as subfile:
            self._server.query(
                url, self._server._session.post, data=subfile, params=params, headers=headers
            )
        return self
    ...
    def removeSubtitles(self, subtitleStream=None, streamID=None, streamTitle=None):
        ...
        if subtitleStream is None:
            try:
                ...
                raise BadRequest(
                    f"Subtitle stream with ID '{streamID}' or title '{streamTitle}' not found."
                ) from None
    ...
    def optimize(
        ...
        if not targetTagID and (not deviceProfile or videoQuality is None):
            raise BadRequest(
                "Unknown quality profile target or missing deviceProfile and videoQuality."
            )
        if targetTagID:
            target = ""
    ...
    def sync(
        self, videoQuality, client=None, clientId=None, limit=None, unwatched=False, title=None
    ):
...
class Movie(Video, Playable, MovieMixins):
    ...
    def _loadData(self, data):
        ...
        self.enableCreditsMarkerGeneration = utils.cast(
            int, data.attrib.get("enableCreditsMarkerGeneration", "-1")
        )
...
class Show(Video, ShowMixins):
    ...
    def download(self, savepath=None, keep_original_name=False, subfolders=False, **kwargs):
        ...
        for episode in self.episodes():
            _savepath = (
                os.path.join(savepath, f"Season {str(episode.seasonNumber).zfill(2)}")
                if subfolders
                else savepath
            )
...
@utils.registerPlexObject
class Season(Video, SeasonMixins):
...
@utils.registerPlexObject
class Episode(Video, Playable, EpisodeMixins):
    ...
    def _defaultSyncTitle(self):
        """Returns str, default title for a new syncItem."""
        return (
            f"{self.grandparentTitle} - {self.parentTitle} - ({self.seasonEpisode}) {self.title}"
        )
...
@utils.registerPlexObject
class Clip(Video, Playable, ClipMixins):
