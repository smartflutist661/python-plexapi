...
class Playlist(PlexPartialObject, Playable, PlaylistMixins):
        ...
        for item in items:
            if item.sourceURI:
                ...
                if serverID not in _servers:
                    try:
                        _servers[serverID] = (
                            self._server.myPlexAccount().resource(serverID).connect()
                        )
                    except NotFound:
        ...
        for server, _items in groupby(items, key=lambda item: item._server):
            ...
            for item in _items:
                if item.listType != self.playlistType:  # pragma: no cover
                    raise BadRequest(
                        f"Can not mix media types when building a playlist: "
    ...
    def _createFromM3U(cls, server, title, section, m3ufilepath):
        ...
        try:
            return (
                server.playlists(sectionId=section.key, guid__endswith=m3ufilepath)[0]
                .editTitle(title)
                .reload()
            )
        except IndexError:
