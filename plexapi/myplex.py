...
class MyPlexAccount(PlexObject):
    ...
    FRIENDSERVERS = (
        "https://plex.tv/api/servers/{machineId}/shared_servers/{serverId}"  # put with data
    )
    ...
    def _loadData(self, data):
        ...
        self.scrobbleTypes = [
            utils.cast(int, x) for x in data.attrib.get("scrobbleTypes").split(",")
        ]
        ...
        self.profileDefaultSubtitleAccessibility = utils.cast(
            int, profile.attrib.get("defaultSubtitleAccessibility")
        )
        self.profileDefaultSubtitleForces = utils.cast(
            int, profile.attrib.get("defaultSubtitleForces")
        )
    ...
    def sonos_speaker(self, name):
        return next(
            (x for x in self.sonos_speakers() if x.title.split("+")[0].strip() == name), None
        )

    def sonos_speaker_by_id(self, identifier):
        return next(
            (x for x in self.sonos_speakers() if x.machineIdentifier.startswith(identifier)), None
        )
    ...
    def cancelInvite(self, user):
        ...
        invite = (
            ...
            else self.pendingInvite(user, includeReceived=False)
        )
        params = {
            "friend": int(invite.friend),
            "home": int(invite.home),
    ...
    def updateFriend(
        ...
        if not user_servers or sectionIds:
            if removeSections is True:
                response_servers = self.query(
                    url, self._session.delete, json=params, headers=headers
                )
            elif "invited_id" in params.get("shared_server", ""):
                response_servers = self.query(
                    url, self._session.post, json=params, headers=headers
                )
            else:
    ...
    def pendingInvite(self, username, includeSent=True, includeReceived=True):
        ...
        for invite in self.pendingInvites(includeSent, includeReceived):
            if (
                ...
                and invite.id
                and username.lower()
                in (invite.username.lower(), invite.email.lower(), str(invite.id))
            ):
                return invite
    ...
    def sync(self, sync_item, client=None, clientId=None):
        ...
        params = {
            ...
            "SyncItem[Policy][value]": str(
                sync_item.policy.value if hasattr(sync_item.policy, "value") else 0
            ),
            "SyncItem[Location][uri]": sync_item.location,
            "SyncItem[MediaSettings][audioBoost]": str(sync_item.mediaSettings.audioBoost),
            "SyncItem[MediaSettings][maxVideoBitrate]": str(
                sync_item.mediaSettings.maxVideoBitrate
            ),
    ...
    def claimToken(self):
        ...
        response = self._session.get(
            "https://plex.tv/api/claim/token.json", headers=self._headers(), timeout=TIMEOUT
        )
        if response.status_code not in (200, 201, 204):  # pragma: no cover
            codename = codes.get(response.status_code)[0]
            errtext = response.text.replace("\n", " ")
    ...
    def addToWatchlist(self, items):
        ...
        for item in items:
            ...
            self.query(
                f"{self.DISCOVER}/actions/addToWatchlist?ratingKey={ratingKey}",
    ...
    def removeFromWatchlist(self, items):
        ...
        for item in items:
            ...
            ratingKey = item.guid.rsplit("/", 1)[-1]
            self.query(
                f"{self.DISCOVER}/actions/removeFromWatchlist?ratingKey={ratingKey}",
                method=self._session.put,
            )
        return self
    ...
    def searchDiscover(self, query, limit=30, libtype=None, providers="discover"):
        ...
        searchResult = next(
            (s.get("SearchResult", []) for s in searchResults if s.get("id") == "external"), []
        )
...
class MyPlexUser(PlexObject):
    ...
    def get_token(self, machineIdentifier):
        try:
            for item in self._server.query(
                self._server.FRIENDINVITE.format(machineId=machineIdentifier)
            ):
...
class Section(PlexObject):
    ...
    def history(self, maxresults=None, mindate=None):
        ...
        return server.history(
            ...
            mindate=mindate,
            accountID=self._server.accountID,
            librarySectionID=self.sectionKey,
        )
...
class MyPlexServerShare(PlexObject):
    ...
    def sections(self):
        ...
        url = MyPlexAccount.FRIENDSERVERS.format(
            machineId=self.machineIdentifier, serverId=self.id
        )
...
class MyPlexResource(PlexObject):
    ...
    def preferred_connections(
        ...
        for connection in self.connections:
            ...
            if self.owned or (not self.owned and not connection.local):
                location = (
                    "relay" if connection.relay else ("local" if connection.local else "remote")
                )
    ...
    def connect(
        ...
        listargs = [
            [cls, url, self.accessToken, self._server._session, timeout] for url in connections
        ]
...
class MyPlexDevice(PlexObject):
    ...
    def connect(self, timeout=None):
        ...
        cls = PlexServer if "server" in self.provides else PlexClient
        listargs = [
            [cls, url, self.token, self._server._session, timeout] for url in self.connections
        ]
...
class MyPlexPinLogin:
    ...
    def run(self, callback=None, timeout=120):
        ...
        self._thread = threading.Thread(
            target=self._pollLogin, name="plexapi.myplex.MyPlexPinLogin"
        )
        self._thread.start()
    ...
    def _pollLogin(self):
        try:
            ...
            while not self._abort and (
                not self._loginTimeout or (time.time() - start) < self._loginTimeout
            ):
                try:
...
class MyPlexJWTLogin:
        ...
        if keyfiles[0] and keyfiles[1]:
            if not overwrite and (os.path.exists(keyfiles[0]) or os.path.exists(keyfiles[1])):
                raise FileExistsError(
                    "Keypair files already exist, set overwrite=True to overwrite them."
                )
    ...
    def _encodeClientJWT(self):
        ...
        headers = {"kid": self._keyID}
        return jwt.encode(payload, key=self._privateJWK, algorithm="EdDSA", headers=headers)
    ...
    def decodePlexJWT(self, verify_signature=True):
        ...
        for plexJWK in reversed(self._getPlexPublicJWK()):
            try:
                return jwt.decode(key=jwt.PyJWK.from_dict(plexJWK), **kwargs)
    ...
    def registerDevice(self):
        ...
        if not self._privateKey or not self._publicKey:
            raise BadRequest(
                "ED25519 private and public keys are required to register device. "
    ...
    def verifyJWT(self, refreshWithinDays=1):
        ...
        try:
            ...
            if decodedJWT["thumbprint"] != self._keyID:
                ...
                return False
            elif decodedJWT["exp"] < int(
                (datetime.now(timezone.utc) + timedelta(days=refreshWithinDays)).timestamp()
            ):
    ...
    def run(self, callback=None, timeout=120):
        ...
        self.finished = False
        self._thread = threading.Thread(
            target=self._pollLogin, name="plexapi.myplex.MyPlexJWTLogin"
        )
        self._thread.start()
...
def _chooseConnection(ctype, name, results):
    ...
    if results:
        log.debug(
            "Connecting to %s: %s?X-Plex-Token=%s", ctype, results[0]._baseurl, results[0]._token
        )
        return results[0]
...
class GeoLocation(PlexObject):
    ...
    def _loadData(self, data):
        ...
        self.coordinates = tuple(
            utils.cast(float, coord)
            for coord in (data.attrib.get("coordinates") or ",").split(",")
        )
        self.country = data.attrib.get("country")
        ...
        self.inPrivacyRestrictedCountry = (
            False
            if inPrivacyRestrictedCountry == "Unknown"
            else utils.cast(bool, inPrivacyRestrictedCountry)
        )
