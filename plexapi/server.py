...
class PlexServer(PlexObject):
    ...
    def _loadData(self, data):
        ...
        self.requestParametersInCookie = utils.cast(
    ...
    def clients(self):
        ...
        for elem in self.query("/clients"):
            ...
            if not port:
                log.warning(
                    "%s did not advertise a port, checking plex.tv.", elem.attrib.get("name")
                )
            ...
            items.append(
                PlexClient(
                    baseurl=baseurl, server=self, token=self._token, data=elem, connect=False
                )
            )
    ...
    def downloadDatabases(self, savepath=None, unpack=False, showstatus=False):
        ...
        filepath = utils.download(
            url, self._token, None, savepath, self._session, unpack=unpack, showstatus=showstatus
        )
        return filepath
    ...
    def runButlerTask(self, task):
        ...
        if task not in validTasks:
            raise BadRequest(f"Invalid butler task: {task}. Available tasks are: {validTasks}")
    ...
    def history(
        self, maxresults=None, mindate=None, ratingKey=None, accountID=None, librarySectionID=None
    ):
    ...
    def bandwidth(self, timespan=None, **kwargs):
        ...
        if timespan is None:
            ...
            timespans = {"seconds": 6, "hours": 4, "days": 3, "weeks": 2, "months": 1}
            try:
                params["timespan"] = timespans[timespan]
            except KeyError:
                raise BadRequest(
                    f"Invalid timespan specified: {timespan}. "
...
class StatisticsResources(PlexObject):
    ...
    def _loadData(self, data):
        ...
        self.processMemoryUtilization = utils.cast(
            float, data.attrib.get("processMemoryUtilization")
        )
        self.timespan = utils.cast(int, data.attrib.get("timespan"))
