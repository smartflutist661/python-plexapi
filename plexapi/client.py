...
class PlexClient(PlexObject):
    ...
    def connect(self, timeout=None):
        ...
        if self._clientIdentifier:
            client = next(
                (x for x in data if x.attrib.get("machineIdentifier") == self._clientIdentifier),
                None,
            )
            if client is None:
                raise NotFound(
    ...
    def sendCommand(self, command, proxy=None, **params):
        ...
        if controller not in self.protocolCapabilities:
            log.debug(
                "Client %s doesn't support %s controller. What your trying might not work",
                self.title,
                controller,
            )
    ...
    def refreshPlayQueue(self, playQueueID, mtype=DEFAULT_MTYPE):
        ...
        Parameters:
            ...
            mtype (str): Media type to take action against (music, photo, video).
        """
        self.sendCommand("playback/refreshPlayQueue", playQueueID=playQueueID, type=mtype)
    ...
    def setStreams(
        self, audioStreamID=None, subtitleStreamID=None, videoStreamID=None, mtype=DEFAULT_MTYPE
    ):
