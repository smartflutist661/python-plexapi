...
class PlexObject(metaclass=PlexObjectMeta):
    ...
    def __repr__(self):
        uid = self._clean(
            self.firstAttr("_baseurl", "ratingKey", "id", "key", "playQueueID", "uri", "type")
        )
    ...
    def findItems(self, data, cls=None, initpath=None, rtag=None, **kwargs):
        ...
        items = (
            MediaContainer[cls](self._server, data, initpath=initpath)
...
class PlexPartialObject(PlexObject):
    ...
    def __getattribute__(self, attr):
        ...
        if attr in _DONT_RELOAD_FOR_KEYS:
            return value
        if attr in USER_DONT_RELOAD_FOR_KEYS:
            return value
        ...
        if self.isFullObject():
            return value
        if isinstance(self, (PlexSession, PlexHistory)):
            return value
        if self._autoReload is False:
            return value
        # Log the reload.
    ...
    def isFullObject(self):
        ...
        return not self.key or (
            parsed_key.path == parsed_initpath.path and query_key <= query_init
        )
    ...
    def delete(self):
        ...
        try:
            ...
            log.error(
                "Failed to delete %s. This could be because you "
                "have not allowed items to be deleted",
                self.key,
            )
            raise
    ...
    def history(self, maxresults=None, mindate=None):
        ...
        return self._server.history(
            maxresults=maxresults, mindate=mindate, ratingKey=self.ratingKey
        )
...
class PlexHistory:
    ...
    def _reload(self, **kwargs):
        ...
        raise NotImplementedError(
            "History objects cannot be reloaded. Use source() to get the source media item."
        )
...
class MediaContainer(
    ...
    def extend(
        ...
        self.totalSize = (
            __iterable.totalSize if __iterable.totalSize is not None else self.totalSize
        ...
        if self.offset is not None and __iterable.offset is not None:
            ...
            self.offset = self.offset if self.offset is not None else __iterable.offset
