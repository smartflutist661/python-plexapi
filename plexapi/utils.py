...
def registerPlexObject(cls):
    ...
    if ehash in PLEXOBJECTS:
        raise Exception(
            f"Ambiguous PlexObject definition {cls.__name__}(tag={cls.TAG}, type={etype}) "
...
def threaded(callback, listargs):
    ...
    for args in listargs:
        ...
        results.append(None)
        threads.append(
            Thread(target=callback, args=args, kwargs=dict(job_is_done_event=job_is_done_event))
        )
    ...
    if value is not None:
        if format:
            try:
                ...
                log.info(
                    'Failed to parse "%s" to datetime as format "%s", defaulting to None',
                    value,
                    format,
                )
                return None
            ...
            try:
                ...
                log.info(
                    'Failed to parse "%s" to datetime as timestamp, defaulting to None', value
                )
                return None
            try:
                ...
                try:
                    ...
                    log.info(
                        'Failed to parse "%s" to datetime as timestamp (out-of-bounds), defaulting to None',
                        value,
                    )
                    return None
    return value
...
def millisecondToHumanstr(milliseconds):
    ...
    return (
        "" if days == 0 else f'{days} day{"s" if days > 1 else ""}, '
    ) + f"{hours:02d}:{mins:02d}:{secs:02d}.{ms:03d}"
...
def downloadSessionImages(
    server, filename=None, height=150, width=150, opacity=100, saturation=100
):  # pragma: no cover
    ...
    for media in server.sessions():
        ...
        if url:
            if filename is None:
                ...
                filename = (
                    f"session_transcode_{media.usernames[0]}_{prettyname}_{int(time.time())}"
                )
...
def plexJWTAuth(
    headers=None, forwardUrl=None, timeout=120, keypair=(None, None), scopes=None
):  # pragma: no cover
