...
class Library(PlexObject):
    ...
    def add(
        ...
        **TheMovieDB Other Video Options** (com.plexapp.agents.themoviedb)
        ...
        part = (
            f"/library/sections?name={quote_plus(name)}&type={type}&agent={agent}"
...
class LibrarySection(PlexObject):
    ...
    def delete(self):
        ...
        try:
            data = self._server.query(
                f"/library/sections/{self.key}", method=self._server._session.delete
            )
    ...
    def _loadFilters(self):
        ...
        """
        _key = (
            "/library/sections/{key}/{filter}?includeMeta=1&includeAdvanced=1"
    ...
    def getFilterType(self, libtype=None):
        ...
        try:
            ...
            raise NotFound(
                f'Unknown libtype "{libtype}" for this library. '
    ...
    def getFieldType(self, fieldType):
        ...
        try:
            ...
            raise NotFound(
                f'Unknown field type "{fieldType}" for this library. '
    ...
    def listFilterChoices(self, field, libtype=None):
        ...
        if isinstance(field, str):
            ...
            try:
                ...
                raise NotFound(
                    f'Unknown filter field "{field}" for libtype "{libtype}". '
    ...
    def _validateFilterField(self, field, values, libtype=None):
        ...
        try:
            filterField = next(
                f for f in self.listFields(libtype) if f.key.split(".")[-1] == field
            )
        except StopIteration:
            for filterType in reversed(self.filterTypes()):
                if filterType.type != libtype:
                    filterField = next(
                        (f for f in filterType.fields if f.key.split(".")[-1] == field), None
                    )
                    if filterField:
                        break
            else:
    ...
    def _validateFieldOperator(self, filterField, operator):
        ...
        try:
            ...
            raise NotFound(
                f'Unknown operator "{operator}" for filter field "{filterField.key}". '
    ...
    def _validateFieldValue(self, filterField, values, libtype=None):
        ...
        try:
            ...
            raise BadRequest(
                f'Invalid value "{value}" for filter field "{filterField.key}", '
    ...
    def _validateFieldValueTag(self, value, filterField, libtype):
        ...
        return next(
            (f.key for f in filterChoices if matchValue in {f.key.lower(), f.title.lower()}), value
        )
    ...
    def _validateSortField(self, sort, libtype=None):
        ...
        try:
            ...
            raise NotFound(
                f'Unknown sort field "{sortField}" for libtype "{libtype}". '
        ...
        if sortDir not in availableDirections:
            raise NotFound(
                f'Unknown sort direction "{sortDir}". Available sort directions: {availableDirections}'
            )
        ...
        for field, values in filters.items():
            if field.lower() in {"and", "or"}:
                if len(filters.items()) > 1:
                    raise BadRequest(
                        "Multiple keys in the same dictionary with and/or is not allowed."
                    )
    ...
    def history(self, maxresults=None, mindate=None):
        ...
        return self._server.history(
            maxresults=maxresults, mindate=mindate, librarySectionID=self.key, accountID=1
        )
    ...
    def saveMultiEdits(self):
        ...
        if not isinstance(self._edits, dict):
            raise BadRequest(
                "Batch multi-editing mode not enabled. Must call `batchMultiEdits()` first."
            )
...
class MusicSection(LibrarySection, ArtistEditMixins, AlbumEditMixins, TrackEditMixins):
    ...
    def stations(self):
        ...
        return next(
            (hub._partialItems for hub in self.hubs() if hub.context == "hub.music.stations"), None
        )
...
class FilteringType(PlexObject):
    ...
    def _manualFilters(self):
        ...
        if self.type == "season":
            additionalFilters.extend([("label", "string", "Labels")])
    ...
    def _manualSorts(self):
        ...
        if self.type == "season":
            additionalSorts.extend([("titleSort", "asc", "Title")])
        elif self.type == "track":
            # Don't know what this is but it is valid
            additionalSorts.extend([("absoluteIndex", "asc", "Absolute Index")])
        elif self.type == "photo":
            additionalSorts.extend([("viewUpdatedAt", "desc", "View Updated At")])
        elif self.type == "collection":
            additionalSorts.extend([("addedAt", "asc", "Date Added")])
    ...
    def _manualFields(self):
        ...
        if self.type == "movie":
            ...
            additionalFields.extend([("label", "tag", "Label")])
        elif self.type == "track":
            additionalFields.extend(
                [
                    ("duration", "integer", "Duration"),
            ...
            additionalFields.extend([("addedAt", "date", "Date Added"), ("label", "tag", "Label")])
        ...
        for field, fieldType, fieldTitle in additionalFields:
            ...
            fieldXML = f'<Field key="{field}" ' f'title="{fieldTitle}" ' f'type="{fieldType}"/>'
...
class ManagedHub(PlexObject):
    ...
    def _loadData(self, data):
        ...
        self.promotedToRecommended = utils.cast(
            bool, data.attrib.get("promotedToRecommended", False)
        )
        self.promotedToSharedHome = utils.cast(
            bool, data.attrib.get("promotedToSharedHome", False)
        )
        ...
        parent = self._parent()
        self.librarySectionID = (
            parent.key if isinstance(parent, LibrarySection) else parent.librarySectionID
        )
