...
class Collection(PlexPartialObject, CollectionMixins):
    ...
    def _loadData(self, data):
        ...
        self.collectionFilterBasedOnUser = utils.cast(
            int, data.attrib.get("collectionFilterBasedOnUser", "0")
        )
    ...
    def filterUserUpdate(self, user=None):
        ...
        user_dict = {"admin": 0, "user": 1}
        key = user_dict.get(user)
        if key is None:
            raise BadRequest(
                f"Unknown collection filtering user: {user}. Options {list(user_dict)}"
            )
    ...
    def addItems(self, items):
        ...
        for item in items:
            if item.type != self.subtype:  # pragma: no cover
                raise BadRequest(
                    f"Can not mix media types when building a collection: {self.subtype} and {item.type}"
                )
    ...
    @classmethod
    def _createSmart(
        cls, server, title, section, limit=None, libtype=None, sort=None, filters=None, **kwargs
    ):
    ...
    def create(
        ...
        **kwargs,
        ...
        if smart:
            ...
            return cls._createSmart(
                server, title, section, limit, libtype, sort, filters, **kwargs
            )
        else:
