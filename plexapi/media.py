...
class Media(PlexObject):
    ...
    def delete(self):
        ...
        try:
            ...
            log.error(
                "Failed to delete %s. This could be because you haven't allowed items to be deleted",
                part,
            )
            raise
...
class TranscodeSession(PlexObject):
    ...
    def _loadData(self, data):
        ...
        self.transcodeHwFullPipeline = utils.cast(
            bool, data.attrib.get("transcodeHwFullPipeline", "0")
        )
...
class MediaTag(PlexObject):
    ...
    def items(self):
        ...
        if not self.key:
            raise BadRequest(
                f"Key is not defined for this tag: {self.tag}. Reload the parent object."
            )
...
class BaseResource(PlexObject):
    ...
    def resourceFilepath(self):
        ...
        if self.ratingKey.startswith("media://"):
            ...
            return str(
                Path(self._parent().metadataDirectory)
                / "Contents"
                / "_combined"
                / self.ratingKey.split("://")[-1]
            )
